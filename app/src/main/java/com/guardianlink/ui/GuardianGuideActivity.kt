...
class GuardianGuideActivity : android.app.Activity() {
    ...
    private fun answerFor(text: String): String = when {
        ...
        text.contains("adult") || text.contains("violence") || text.contains("gambl") || text.contains("social media") || text.contains("short") || text.contains("youtube") || text.contains("browser") || text.contains("keyword") -> "Parents can turn on Adult content, Graphic violence, Gambling, and Social media website filters in Safety. Guardian Link Family Browser checks known domains and clear page/search terms without a paid API. Make it the child phone’s default browser for ordinary links. It cannot inspect content inside the official YouTube, Chrome, or installed social-media apps."
