...
data class FamilyChatMessage(val id: String, val senderRole: String, val body: String, val audioPath: String?, val createdAt: String)
...
class ParentApi(private val session: ParentSession) {
    ...
    fun chatMessages(deviceId: String): List<FamilyChatMessage> = get("/rest/v1/family_chat_messages?device_id=eq.$deviceId&select=id,sender_role,body,audio_path,created_at&order=created_at.desc&limit=80")?.let { rows ->
        (0 until rows.length()).map { index -> chatRecord(rows.getJSONObject(index)) }.reversed()
    } ?: emptyList()

    fun sendChatMessage(familyId: String, deviceId: String, body: String, audioPath: String? = null): Boolean {
        if (body.length > 600) return false
        return post("/rest/v1/family_chat_messages", JSONObject().apply {
    ...
    private fun chatRecord(row: JSONObject) = FamilyChatMessage(row.getString("id"), row.getString("sender_role"), row.optString("body"), row.optString("audio_path").ifBlank { null }, row.getString("created_at"))
